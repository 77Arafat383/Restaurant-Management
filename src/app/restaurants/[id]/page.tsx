...
export default function RestaurantDetailPage() {
  ...
  const canDeleteDish = currentUser && (
    currentUser.role === 'ADMIN' || 
    (currentUser.role === 'RESTAURANT_MANAGER' && currentUser.restaurantId === restaurantId)
  );

  const handleDeleteFood = async (foodId: string) => {
    if (!window.confirm('Are you sure you want to remove this dish from the menu?')) return;
    try {
      const res = await fetch(`/api/foods?id=${foodId}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        setAllFoods(prev => prev.filter(f => f.id !== foodId));
        setToMessage('Dish deleted successfully');
                      ...
                      <div className="flex items-center gap-2">
                        {canDeleteDish && (
                          <button
                            ...
                            className="p-2 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-55 border border-transparent hover:border-red-100 transition-all shrink-0"
                            title="Delete Dish"
                          >
